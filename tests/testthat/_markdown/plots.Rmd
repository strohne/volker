...
### 1a. One variable: Frequency plot of adopter
```{r}
data |> 
  plot_counts_one(adopter)
```

### 1b. One variable: Frequency plot of binary data
```{r}
data |> 
...
### cb. One variable: Frequency plot of a focus category
```{r}
data |> 
  plot_counts_one(adopter, category="I try new offers immediately")
```


### 2a. Multiple items: Frequency plot for adoption factors
```{r}
plot_counts_items(data, starts_with("cg_adoption_"), numbers = "p")
```

### 2b. Multiple items: Frequency plot for binary adoption values
```{r}
data |> 
...
data |> 
  mutate(adopter = factor(adopter, levels = rev(adopter))) |> 
