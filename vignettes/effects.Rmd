...
data_coder2 <- data |> 
  ...
  mutate(
    ...
    topic_test = str_detect(cg_activities, "probier|spass|freude|idee|interesse"),
    coder="c2"
  )
...
## 2. Reliability for a single item
```{r}

effect_counts(data_coded, topic_schreiben, coder, ids = case, method="agree")
...
## 5. Reliability for multiple items
```{r}
effect_counts(data_coded, starts_with("topic_"), coder, ids = case, method="agree")
```
