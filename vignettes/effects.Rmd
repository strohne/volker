...
## 2. Cross table
```{r}
# TODO: Output a heatmap with npmi values
...
data %>% 
  mutate(across(starts_with("cg_act_"), ~ ifelse(is.na(.), FALSE, TRUE))) %>% 
  effect_counts(starts_with("cg_act_"), starts_with("cg_act_"), method="npmi")
...
data |> 
  filter(sd_gender != "diverse") |> 
  model_metrics_tab(use_work, categorical = c(sd_gender, adopter), metric = c(starts_with("cg_adoption_advantage"), sd_age))

data |> 
  filter(sd_gender != "diverse") |> 
  model_metrics_plot(use_work, categorical = c(sd_gender, adopter), metric = c(starts_with("cg_adoption_advantage"), sd_age))
