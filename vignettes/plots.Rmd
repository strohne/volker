...
### 2. Two variables: Stacked frequency plot and heatmap of innovator type by gender
```{r}
...
# The tiles=TRUE parameter generates a heatmap,
# the npmi=TRUE parameter uses npmi values as base,
# significance stars of a value-wise fisher test 
# are added by including "stars" in the numbers parameter.
plot_counts(data, sd_gender, adopter, tiles = TRUE, npmi = TRUE, numbers=c("n", "stars"))
...
plot_counts(data, starts_with("cg_adoption_advantage"), starts_with("cg_adoption_social"), method="cramer")

data %>% 
  mutate(across(starts_with("cg_act_"), ~ ifelse(is.na(.), FALSE, TRUE))) %>% 
  plot_counts(starts_with("cg_act_"), starts_with("cg_act_"), method="npmi")
