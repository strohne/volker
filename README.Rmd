...
-   *Individual, grouped or correlated?*\
    ...
    The effects-functions calculate effect sizes and statistical tests 
...
You can calculate effect sizes and conduct basic statistical tests using  `effects_counts()` and `effects_metrics()`. Effect calculation is included in the reports if you request it by the effects-parameter of `report_counts()` or `report_metrics()`. 
...
automatically recoded to missing values within all plot, tab, effects, and report
