...
All functions take a data frame as their first argument, followed by column selections, and optionally a grouping column. Examples (hint: replace "tab_" by "plot_" to reproduce the examples above):
...
- Grouped categorical variable: `tab_counts(data, in_adoption, sd_geschlecht)`  
...
The attributes to store labels follow a format used, for example,
on [Sosci Survey](https://www.soscisurvey.de/). That's why you can directly throw in
...
Indexes (=mean of multiple items) can be added using `add_idx()` manually
...
**Hint:**
To get the vignettes (= guides to the package, they help getting started), 
you need to build them in the install process. Only then, you'll find them
linked in the help index page.
```
...
**Citation**  
Jünger, J. (2024). volker: High-level functions for summarising, charting and reporting survey data. R package version 1.0.
