...
[![CRAN status](https://www.r-pkg.org/badges/version/volker)](https://CRAN.R-project.org/package=volker)
<!-- badges: end -->
...
- *One variable or item batteries?*.  
  ...
  To summarise multiple items send a column selection to the functions by using tidyselect mechanisms such as `starts_with()`. Indexes are automatically calculated by the `report`-functions or can be explicitly created using `idx_add()`.
...
loose labels on their way.
In this case, store the labels (in the codebook attribute of the data frame)
before the operation and resotre them afterwards:
...
The labeling mechanisms follow a technique used, for example,
on [SoSci Survey](https://www.soscisurvey.de/). Sidenote for techies: Labels are stored in the column attributes. 
That's why you can directly throw in
...
*Please note:* The values `-9` and `[NA] nicht beantwortet` are automatically recoded to missing values
within all plot, tab and report functions. Missing control is on the list for the next package version.

## Index calculation

Indexes (=mean of multiple items) can be added using `idx_add()` manually
and are automatically calculated in report functions. 
...
Alternatively, you can install the latest development version from GitHub using remotes
(if asked, skip the updates):
...
The beta version used in the statistics course in winter 2023/24 at the University of Münster
can be installed using remotes from the beta branch (if asked, skip the updates):
