...
lda_get_termrelevance <- function(fit, lambda=0.6) {
  ...
  # Get topic-word probabilities  matrix.
  # Each row is a topic, each column is a unique word in the vocabulary.
  # Each value is the probability of that word being generated by the corresponding topic.
  lda_phi <- modeltools::posterior(fit)$terms %>% as.matrix()
  lda_rel <- relevance(lda_phi, lambda)
