...
agree_tab <- function(data, cols, coders, ids = NULL, method = "reliability", labels = TRUE, clean = TRUE, ...) {
  ...
  result <- .agree_items(data, {{ cols }}, {{ coders }}, ids = {{ ids }}, method = method, labels = labels)

  # 3. Labels
  result$item2 <- NULL
  if (nrow(result) > 1) {
    result <- .replace_first_col_labels(result)
  } else {
    colnames(result)[1] <- "item"
  }
  ...
  result <- expand.grid(
    ...
    dplyr::mutate(
      .test = purrr::pmap(
        ...
        \(x, y, z) .agree_one(data[[x]], data[[y]], ids = data[[z]], category = category, method = method)
      )
    )
...
#' Calculate agreement scores for one item
#'
#' @keywords internal
#'
#' @param x Codings.
...
#' @return A list with agreement coefficients.
#'         For F1 calculations, macro statistics are returned (i.e. average of statistics for each category).
.agree_one <- function(x, y, ids = NULL, category = NULL, method) {
  ...
  if (method == "reliability") {
    ...
    if (all(as.character(cat_vec) %in% c("TRUE", "FALSE"))) {
      category <- "TRUE"
    }
